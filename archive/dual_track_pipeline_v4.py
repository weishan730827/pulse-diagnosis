...
def cross_validate_v4(bajia_report, chen_report, chen_scored, symptoms, pulses_str=""):
    ...
    for line in bajia_report.split("\n"):
        matched = False
        # 张仲景特殊处理：【零·根本层】张仲景
        if "【零·根本层】" in line and "张仲景" in line:
            ...
            matched = True
            continue
        for name, key in section_names.items():
            # 匹配 "一、姚梅龄" "四、胡希恕" "八、黄元御" "十一、郑钦安" 等各种编号
            if re.search(rf"[\d一二三四五六七八九十]+、\s*{name}", line):
                if current_section and section_lines:
                    bajia_sections[current_section] = "\n".join(section_lines)
                current_section = key
                section_lines = [line.strip()]
    ...
    def extract_formulas(text, top_n=5):
        ...
        for m in re.finditer(r'→\s*(?:方[：:]?\s*)?([\u4e00-\u9fff]+汤|[\u4e00-\u9fff]+丸|[\u4e00-\u9fff]+散)', text):
            ...
            if not any(f[0] == name for f in found):
                found.append((name, 0))
        # 治则文本中的方剂（者XX汤、用XX汤、宜XX汤 等）
        for m in re.finditer(r'(?:者|用|宜)\s*([\u4e00-\u9fff]+汤|[\u4e00-\u9fff]+丸|[\u4e00-\u9fff]+散)', text):
            name = m.group(1)
            if not any(f[0] == name for f in found):
                found.append((name, 0))
        # 类方列表：升陷类方：升陷汤 / 回阳升陷汤 / ...  pattern
        for m in re.finditer(r'(?:类方|方向)[：:]\s*(.+?)(?:\n|$)', text):
            for subm in re.finditer(r'([\u4e00-\u9fff]+(?:汤|丸|散|丹))', m.group(1)):
                name = subm.group(1)
                if not any(f[0] == name for f in found):
                    found.append((name, 0))
        return found[:top_n]
    ...
    systems = [
        ("张仲景（本经·六经定纲）", zzj_full,
         [r"六经归属[：:].*", r"治则大法[：:].*", r"少阴.*病", r"太阳.*病", r"原文[：:].*"],
         []),
        ("姚梅龄（脉象分析）", yao_full,
    ...
    for label, full_text, conclusion_patterns, default_formulas in systems:
        ...
        # 张锡纯方向修正：升陷汤类方适用时，移除镇肝方向方剂
        if "张锡纯" in label:
            if "升陷汤类方适用" in full_text or "大气下陷指征" in full_text or "忌攻降" in full_text:
                formulas = [f for f in formulas if "镇肝" not in f[0] and "参赭" not in f[0]]
            elif "镇肝" in full_text:
                formulas = [f for f in formulas if "升陷" not in f[0]]
