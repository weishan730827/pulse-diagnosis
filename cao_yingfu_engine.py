...
class CaoYingFuEngine:
    ...
    def diagnose(self, checked_ids: List[str]) -> Dict:
        ...
        返回格式:
        {
            "matched_formulas": [{"fang": "桂枝汤", "count": 3, "symptoms": [...]}, ...],
            "unmatched_ids": [...],
            "total_checked": int,
            "total_matched": int
        }
        """
        fang_counter: Dict[str, Dict] = {}  # fang_name → {count, symptoms}
        ...
        for sid in checked_ids:
            ...
            for f in fangs:
                if f not in fang_counter:
                    fang_counter[f] = {"fang": f, "count": 0, "symptoms": []}
                fang_counter[f]["count"] += 1
                fang_counter[f]["symptoms"].append({"id": sid, "text": text})

        # 按命中次数降序排列
        ranked = sorted(fang_counter.values(), key=lambda x: x["count"], reverse=True)
    ...
    def format_result(self, result: Dict) -> str:
        ...
        if result["matched_formulas"]:
            lines.append("【匹配经方】（按命中次数排序）")
            for i, entry in enumerate(result["matched_formulas"], 1):
                lines.append(f"  {i}. {entry['fang']} (命中 {entry['count']} 次)")
                for sym in entry["symptoms"]:
                    lines.append(f"     └ [{sym['id']}] {sym['text']}")
        else:
            lines.append("【无匹配经方】")
