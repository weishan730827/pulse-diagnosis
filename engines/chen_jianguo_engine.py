...
class ChenJianGuoEngine:
    ...
    def _bilateral_analysis(self, grid_values: Dict[str, str]) -> Dict:
        """左右对比：陈建国脉诊核心——同一位左右差异即是病机所在"""
        pairs = [
        ...
        return {
            "findings": findings,
            "summary": "左右均衡" if not findings else f"共 {len(findings)} 处左右不均衡"
        }
