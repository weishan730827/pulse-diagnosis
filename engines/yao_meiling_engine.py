#!/usr/bin/env python3
"""
姚梅龄脉学辨证匹配引擎 v3（对接仲景基座）
方法：纯脉诊辨证。知医邦28脉10维量化全量输入→分层分析→病机推演。
      v3：方剂全部对接仲景基座（银翘散→麻杏甘石汤）。
"""
...
CHECKBOX_PATH = os.path.join(_OUTPUT_DIR, "..", "checkbox_tables", "姚梅龄脉学辨证勾选表_v1.json")


class YaoMeiLingEngine:
    """姚梅龄引擎：脉诊三层解析（v3 对接仲景基座）"""
    ...
    def _load_skills(self, filename: str) -> dict:
        path = os.path.join(_OUTPUT_DIR, "..", "skills", filename)
    ...
    def _analyze_compounds(self, all_pulses):
        findings = []
        ...
        for pos, pulses in pos_pulses.items():
            for combo in combinations(sorted(pulses), 2):
                ...
                if key in self.COMPOUND_RULES and key not in seen:
                    ...
                    findings.append({"position": pos, "pulses": list(key), **rule})
                    seen.add(key)
        return findings

    def _judge_bingshi(self, all_pulses):
    ...
    def _recommend(self, bingwei, hanre, xushi):
        """全仲景方剂推荐"""
        formulas = []
        if bingwei == "表" and xushi == "实":
            formulas.append("麻黄汤（表寒实证）" if hanre == "寒" else "麻杏甘石汤（表热实证）")
    ...
    def format_result(self, result: Dict) -> str:
        lines = []
        lines.append("=== 姚梅龄脉学辨证结果（v3 仲景方库） ===")
