        ...
        for section in self.data["step3_兼证细化"]["sections"]:
            ...
            for item in section.get("items", []):
                self._item_map[item["id"]] = {
                    ...
                    "direction": direction,
                    "maps_to": item.get("maps_to", ""),
                    "is_refined": True,  # 兼证细化优先级 > 基础症状
                }

    def diagnose(self, checked_ids: List[str]) -> Dict:
        """判定升降 + 推荐方剂（兼证细化优先于基础方）"""
        xiaxian_items: List[Dict] = []
        shangni_items: List[Dict] = []
        # 分基础症状 vs 兼证细化
        xiaxian_base_fangs: Dict[str, int] = {}
        xiaxian_refined_fangs: Dict[str, int] = {}
        shangni_base_fangs: Dict[str, int] = {}
        shangni_refined_fangs: Dict[str, int] = {}
        ...
        for sid in checked_ids:
            ...
            if direction == "大气下陷":
                ...
                if maps_to:
                    if is_refined:
                        xiaxian_refined_fangs[maps_to] = xiaxian_refined_fangs.get(maps_to, 0) + 1
                    else:
                        xiaxian_base_fangs[maps_to] = xiaxian_base_fangs.get(maps_to, 0) + 1
            elif direction == "气机上逆":
                shangni_items.append(item)
                if maps_to:
                    if is_refined:
                        shangni_refined_fangs[maps_to] = shangni_refined_fangs.get(maps_to, 0) + 1
                    else:
                        shangni_base_fangs[maps_to] = shangni_base_fangs.get(maps_to, 0) + 1
        ...
        if xiaxian_score > shangni_score:
            result = "大气下陷"
            # 兼证细化优先，兜底基础方
            if xiaxian_refined_fangs:
                formulas = sorted(xiaxian_refined_fangs.items(), key=lambda x: x[1], reverse=True)
            elif xiaxian_base_fangs:
                formulas = sorted(xiaxian_base_fangs.items(), key=lambda x: x[1], reverse=True)
            else:
                formulas = [("升陷汤", 1)]
        elif shangni_score > xiaxian_score:
            result = "气机上逆"
            if shangni_refined_fangs:
                formulas = sorted(shangni_refined_fangs.items(), key=lambda x: x[1], reverse=True)
            elif shangni_base_fangs:
                formulas = sorted(shangni_base_fangs.items(), key=lambda x: x[1], reverse=True)
            else:
                formulas = [("参赭镇气汤", 1)]
        else:
        ...
        return {
            ...
            "refined_fangs": bool(xiaxian_refined_fangs or shangni_refined_fangs),
            "formulas": formulas
        }
    ...
    def format_result(self, result: Dict) -> str:
        ...
        if result["formulas"]:
            ...
            for f, c in result["formulas"]:
                tag = "★" if result.get("refined_fangs") else "  "
                lines.append(f"  {tag} {f} (兼证命中 {c})" if tag == "★" else f"  {f} (症状信号 {c})")
