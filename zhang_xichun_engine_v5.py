...
CHECKBOX_PATH = os.path.join(_OUTPUT_DIR, "张锡纯气机升降辨证勾选表_v2.json")
...
# 仲景方池——来自张锡纯六经气化解读的对接方
ZHONGJING_POOL = {
    "桂枝汤", "麻黄汤", "白虎汤", "大承气汤", "栀子豉汤",
    "小柴胡汤", "理中汤", "四逆汤", "黄连阿胶汤", "乌梅丸"
}


class ZhangXiChunEngine:
    """张锡纯引擎 v5：先判升降→自创方池 + 六经气化→仲景方池，双轨合一评分"""
    ...
    def diagnose(self, checked_ids: List[str]) -> Dict:
        ...
        if zhongjing_items:
            chain.append(f"第五步·六经气化映射：{len(zhongjing_items)}项六经症状勾选")
    ...
    def format_result(self, result: Dict) -> str:
        lines = []
        lines.append("=== 张锡纯气机升降辨证结果（v5 自创方+仲景方双轨合一）===")
        lines.append("")
        lines.append("【辨证链】")
...
if __name__ == "__main__":
    ...
    print(f"已加载 {len(engine._id_map)} 项（含六经方证映射）")
    print("=" * 60)
    ...
    print()
    print("=" * 60)
    print("测试2（仲景方向）: 口苦咽干目眩 + 胸胁苦满 + 脉弦 + 默默不欲饮食 → 小柴胡汤")
    ...
    print()
    print("=" * 60)
    print("测试3（双轨合一）: 气短 + 乏力 + 脉沉弱 + 发热恶风汗出 → 升陷汤 vs 桂枝汤")
    test3 = ["zxc_xiaxian_qi_duan", "zxc_xiaxian_fa_li", "zxc_mai_chen_ruo",
